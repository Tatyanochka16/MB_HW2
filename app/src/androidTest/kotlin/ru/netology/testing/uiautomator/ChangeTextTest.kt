...
class ChangeTextTest {
    ...
    private val textToSetNewString = "New String"
...
//    }
    ...
    @Test
    fun testChangeTextForEmpty() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        //Save initial title text
        val InitialText = device.findObject(By.res(packageName, "textToBeChanged")).text
        // Try to change for empty string
    ...
    @Test
    fun testChangeTextForActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        device.findObject(By.res(packageName, "userInput")).text = textToSetNewString
        device.findObject(By.res(packageName, "buttonActivity")).click()

        waitForPackage(packageName)

        val result = device.findObject(By.res(packageName, "text")).text
        assertEquals(result, textToSetNewString)
